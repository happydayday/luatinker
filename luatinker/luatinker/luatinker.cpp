...
bool luatinker::print_table( lua_State* L, int idx )
{
    lua_pushnil( L );
    while( lua_next(L, idx) != 0 )
    {
        ...
        switch(valueType)
        {
            ...
            case LUA_TUSERDATA :
                {
                    std::cout<<"Value:"<<"userdata" << std::endl;
                    break;
                }
            ...
            case LUA_TTABLE:
                {
                    ...
                    if ( !print_table(L, index ))
                    {
                        print_error( L, "print_table error in  print_table,error occured" );
                        lua_pop( L, 1 );
                        continue;
                    }
                    break;
...
int luatinker::meta_get( lua_State *L )
{
    ...
    else if( lua_isnil( L, -1 ) )
    {
        ...
        {
            // stack: 1.类(userdata) 2.变量(string) 3.meta(table) 4.nil
            lua_pushfstring( L, "meta_get can't find '%s' class variable.(forgot registering class variable ?)", lua_tostring( L, 2 ) );
            lua_error( L );
        }
    }
