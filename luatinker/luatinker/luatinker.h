...
namespace luatinker
{
    ...
    bool    print_table( lua_State* L, int idx );
    ...
    template<typename T>
        struct object2lua
        {
            static void invoke( lua_State *L, T val )
            {
                ...
                class_<typename class_type<T>::type>::push_meta( L );
    ...
    template<typename RVal>
        RVal call( lua_State* L, const char* name )
        {
            ...
            {
                lua_pcall( L, 0, ret_<RVal>::value, errfunc );
            }
            else
            {
    ...
    template <typename RVal, typename T1>
        RVal call( lua_State *L, const char * name, T1 arg )
        {
            ...
            {
                push( L, arg );
                lua_pcall( L, 1, ret_<RVal>::value, errfunc );
            }
            else
            {
                print_error( L, "lua_tinker::call() attempt to call global `%s'(not a function)", name );
    ...
    template <typename RVal, typename T1, typename T2>
        RVal call( lua_State *L, const char * name, T1 arg1, T2 arg2 )
        {
            ...
            if( lua_isfunction( L, -1 ) )
            {
                push( L, arg1 );
                push( L, arg2 );
                lua_pcall( L, 2, ret_<RVal>::value, errfunc );
            }
            else
            {
                print_error( L, "lua_tinker::call() attempt to call global `%s'(not a function)", name );
    ...
    template <typename RVal, typename T1, typename T2, typename T3>
        RVal call( lua_State *L, const char * name, T1 arg1, T2 arg2, T3 arg3 )
        {
            ...
            if( lua_isfunction( L, -1 ) )
            {
                ...
                push( L, arg2 );
                push( L, arg3 );
                lua_pcall( L, 3, ret_<RVal>::value, errfunc );
            }
            else
            {
                print_error( L, "lua_tinker::call() attempt to call global `%s'(not a function)", name );
    ...
    template< typename T >
        struct class_
        {
            class_( lua_State * ls, const char * name )
                :L( ls )
            {
                _name( name );
            ...
            template<typename P>
                class_<T>& inh()
                {
                    ...
                    if( lua_istable( L, -1 ) )
                    {
                        ...
                        class_<typename class_type<P>::type>::push_meta( L );
                        lua_rawset( L, -3 );
                    }
        ...
        // 静态成员函数
        template<typename F>
            class_<T>& defstatic( const char* name, F func )
            {
                class_<typename class_type<T>::type>::push_meta( L );
                if ( lua_istable( L, -1 ) )
                {
        ...
        template<typename BASE, typename VAR>
            class_<T>& prop( const char* name, VAR BASE::*val )
            {
                // 获取类table
                class_<typename class_type<T>::type>::push_meta( L );
