...
template<>
inline Vec3<uint8_t>::Vec3(const Vec3<float> &copy) { x = (uint8_t) floor(copy.x +0.5f); y = (uint8_t) floor(copy.y + 0.5f); z = (uint8_t) floor(copy.z + 0.5f); }

template<>
inline Vec3<uint8_t>::Vec3(const Vec3<double> &copy) { x = (uint8_t) floor(copy.x+0.5); y = (uint8_t) floor(copy.y+0.5); z = (uint8_t) floor(copy.z + 0.5); }

template<>
inline Vec3<uint8_t>::Vec3(const Vec3<int32_t> &copy) { x = (uint8_t) copy.x; y = (uint8_t) copy.y; z = (uint8_t) copy.z; }

template<>
inline Vec3<int8_t>::Vec3(const Vec3<float> &copy) { x = (int8_t) floor(copy.x +0.5f); y = (int8_t) floor(copy.y + 0.5f); z = (int8_t) floor(copy.z + 0.5f); }

template<>
inline Vec3<int8_t>::Vec3(const Vec3<double> &copy) { x = (int8_t) floor(copy.x+0.5); y = (int8_t) floor(copy.y+0.5); z = (int8_t) floor(copy.z + 0.5); }

template<>
inline Vec3<int8_t>::Vec3(const Vec3<int32_t> &copy) { x = (int8_t) copy.x; y = (int8_t) copy.y; z = (int8_t) copy.z; }

template<>
inline Vec3<uint16_t>::Vec3(const Vec3<float> &copy) { x = (uint16_t) floor(copy.x +0.5f); y = (uint16_t) floor(copy.y + 0.5f); z = (uint16_t) floor(copy.z + 0.5f); }

template<>
inline Vec3<uint16_t>::Vec3(const Vec3<double> &copy) { x = (uint16_t) floor(copy.x+0.5); y = (uint16_t) floor(copy.y+0.5); z = (uint16_t) floor(copy.z + 0.5); }

template<>
inline Vec3<uint16_t>::Vec3(const Vec3<int32_t> &copy) { x = (uint16_t) copy.x; y = (uint16_t) copy.y; z = (uint16_t) copy.z; }

template<>
inline Vec3<int16_t>::Vec3(const Vec3<float> &copy) { x = (int16_t) floor(copy.x +0.5f); y = (int16_t) floor(copy.y + 0.5f); z = (int16_t) floor(copy.z + 0.5f); }

template<>
inline Vec3<int16_t>::Vec3(const Vec3<double> &copy) { x = (int16_t) floor(copy.x+0.5); y = (int16_t) floor(copy.y+0.5); z = (int16_t) floor(copy.z + 0.5); }

template<>
inline Vec3<int16_t>::Vec3(const Vec3<int32_t> &copy) { x = (int16_t) copy.x; y = (int16_t) copy.y; z = (int16_t) copy.z; }

template<>
inline Vec3<int32_t>::Vec3(const Vec3<float> &copy) { x = (int) floor(copy.x +0.5f); y = (int) floor(copy.y + 0.5f); z = (int) floor(copy.z + 0.5f); }

template<>
inline Vec3<int32_t>::Vec3(const Vec3<double> &copy) { x = (int) floor(copy.x+0.5); y = (int) floor(copy.y+0.5); z = (int) floor(copy.z + 0.5); }

template<>
inline Vec3<int32_t>::Vec3(const Vec3<int32_t> &copy) { x = (int) copy.x; y = (int) copy.y; z = (int) copy.z; }

template<>
inline Vec3<uint32_t>::Vec3(const Vec3<float> &copy) { x = (uint32_t) floor(copy.x +0.5f); y = (uint32_t) floor(copy.y + 0.5f); z = (uint32_t) floor(copy.z + 0.5f); }

template<>
inline Vec3<uint32_t>::Vec3(const Vec3<double> &copy) { x = (uint32_t) floor(copy.x+0.5); y = (uint32_t) floor(copy.y+0.5); z = (uint32_t) floor(copy.z + 0.5); }

template<>
inline Vec3<uint32_t>::Vec3(const Vec3<int32_t> &copy) { x = (uint32_t) copy.x; y = (uint32_t) copy.y; z = (uint32_t) copy.z; }
...
template<>
inline Vec3<float>::Vec3(const Vec3<int32_t> &copy) { x = (float) copy.x; y = (float) copy.y; z = (float) copy.z; }
...
template<>
inline Vec3<double>::Vec3(const Vec3<int32_t> &copy) { x = (double) copy.x; y = (double) copy.y; z = (double) copy.z; }
...
template<typename Type>
inline Vec3<Type> &Vec3<Type>::round() { x = (Type) floorf(x+0.5f); y = (Type) floorf(y+0.5f); z = (Type) floorf(z+0.5f); return *this; }

template<typename Type>
inline Vec3<Type> Vec3<Type>::round(const Vec3<Type>& vector) { Vec3<Type> dest(vector); dest.round(); return dest; }

template<typename Type>
inline Vec3<Type> Vec3<Type>::cross(const Vec3<Type>& v1, const Vec3<Type>& v2) { Vec3<Type> tmp = Vec3<Type>(v1.y * v2.z - v1.z * v2.y, v1.z * v2.x - v1.x * v2.z, v1.x * v2.y - v1.y * v2.x); return tmp; }

template<typename Type>
inline Vec3<Type> &Vec3<Type>::cross(const Vec3<Type>& v) { *this = cross(*this, v); return *this; }

template<typename Type>
inline Vec3<Type> Vec3<Type>::reflect(const Vec3<Type>& incident, const Vec3<Type>& normal) { return incident - normal.dot(incident) * normal * Type(2); }

// For integers
template<typename Type>
inline Type Vec3<Type>::distance(const Vec3<Type>& vector) const { float value_x, value_y, value_z; value_x = x - vector.x; value_y = y - vector.y; value_z = z - vector.z; return (Type) floor(sqrt(value_x*value_x + value_y*value_y+value_z*value_z)+0.5f); }

// For floats
template<>
inline float Vec3<float>::distance(const Vec3<float> &vector) const { float value_x, value_y, value_z; value_x = x - vector.x; value_y = y - vector.y; value_z = z - vector.z; return sqrt(value_x*value_x + value_y*value_y+value_z*value_z); }

// For doubles
template<>
inline double Vec3<double>::distance(const Vec3<double> &vector) const { double value_x, value_y, value_z; value_x = x - vector.x; value_y = y - vector.y; value_z = z - vector.z; return sqrt(value_x*value_x + value_y*value_y+value_z*value_z); }
